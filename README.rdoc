...
Here's an example config/deploy.rb that uses a few helpers:
  ...
  require 'capistrano-abril/config_files'     # Symlink to config file
...
At the end the following link is created:
    /abd/app/mysite/current/structure

pointing to the latest release of a site-structure deploy.
...
structure inside /abd/app/mysite/structure:
    ...
    /abd/app/mysite1/structure/current -> releases/2011-03-02-1445
...
=== Simple Rails Application
  ...
  #
    ...
    require 'capistrano-abril/branch'       # Deploy by BRANCH/TAG/commit
    require 'capistrano-abril/bundler'      # Runs "bundle install" correctly
    require 'capistrano-abril/config_files' # Symlinks to config/
    require 'capistrano-abril/git'          # Git defaults
    require 'capistrano-abril/multistage'   # Support to multistage scenarios
    require 'capistrano-abril/passenger'    # Support for Apache passenger
    require 'capistrano-abril/symlinks'     # Symlinks to RAILS_ROOT
    require 'capistrano-abril/setup'        # deploy:setup as supposed to be.
    require 'capistrano-abril/version'      # Record the version number after deploying
...
=== Alexandria
  ...
  ###                                                         |  ###
  ### Site-Reference                                          |  ### Site-Structure
  ###                                                         |  ###
                                                              |
  #                                                           |  #
  # Deploy Settings                                           |  # Deploy Settings
  #                                                           |  #
  set :user            , "siteref"                            |  set :user            , "siteref"
  set :port            , 22                                   |  set :port            , 22
                                                              |
  set :application     , "site-reference"                     |  set :application     , "site-reference"
  set :repository      , "git:/temp/alx-demo-reference.git"   |  set :repository      , "git:/temp/alx-demo-structure.git"
  set :repository_cache, "git-#{application}"                 |  set :repository_cache, "git-#{application}"
                                                              |
  set :structure_path  , "/abd/app/#{application}/structure"  |  set :structure_path  , "/abd/app/#{application}/structure"
                                                              |
  #                                                           |  #
  # Tasks                                                     |  # Tasks
  #                                                           |  #
  require 'capistrano-abril/abril'                            |  require 'capistrano-abril/abril'
  require 'capistrano-abril/branch'                           |  require 'capistrano-abril/branch'
  require 'capistrano-abril/bundler'                          |  require 'capistrano-abril/git'
  require 'capistrano-abril/config_files'                     |  require 'capistrano-abril/multistage'
  require 'capistrano-abril/git'                              |  require 'capistrano-abril/setup'
  require 'capistrano-abril/multistage'                       |  require 'capistrano-abril/version'
  require 'capistrano-abril/passenger'                        |
  require 'capistrano-abril/symlinks'                         |
  require 'capistrano-abril/setup'                            |
  require 'capistrano-abril/version'                          |
                                                              |
  require 'capistrano-abril/site-reference'                   |  require 'capistrano-abril/site-structure'
                                                              |
  after "deploy:symlink", "deploy:cleanup"                    |  after "deploy:symlink", "deploy:cleanup"
